...
function PSUs() {
	...
	this.getbyid = function(formno, res){
		connection.acquire(function(err, con){
			con.query('SELECT * FROM psus where tab_form_no = ?', formno , function(err, result){
				con.release();
				res.send(result);
			});
		});
	...
	this.create = function(form, res){
		fs.appendFile("./psus.json", new Date+" - "+JSON.stringify(form)+"\r\n", function(err) {
    if(err) {
    ...
    console.log("The file was saved!");
		...
		connection.acquire(function(err, con){
			...
			con.query('INSERT INTO psus SET ?', form[i], function(err, result){
	...
	this.update = function(form, res){
		connection.acquire(function(err, con){
			con.query('UPDATE psus SET ? where tab_form_no = ?', [form, form.formno], function(err, result){
				con.release();
				if(err){
