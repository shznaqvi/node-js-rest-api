var connection = require('../../enrichconn');
...
function Districts() {
	this.get = function(res){
		connection.acquire(function(err, con){
			con.query('SELECT * FROM districts', function(err, result){
				con.release();
				res.send(result);
			});
		});
	...
	this.create = function(form, res){
		fs.appendFile("./districts.json", new Date+" - "+JSON.stringify(form)+"\r\n", function(err) {
    if(err) {
    ...
    console.log("The file was saved!");
		...
		connection.acquire(function(err, con){
			...
			con.query('INSERT INTO districts SET ?', form[i], function(err, result){
	...
	this.update = function(form, res){
		connection.acquire(function(err, con){
			con.query('UPDATE districts SET ? where tab_form_no = ?', [form, form.formno], function(err, result){
				con.release();
				if(err){
